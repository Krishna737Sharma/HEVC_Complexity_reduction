...
# test_loader, test_indices = create_subset_dataloader(test_file_path, TESTSET_MAXSIZE, 80000, BATCH_SIZE, shuffle=True)

# Set device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Define test file path and maximum size (adjust these as needed)
test_file_path = "/home/m1/23CS60R16/Data/AI_Test_196350.dat_shuffled"  # update with your actual test file
TESTSET_MAXSIZE = 196350  # example value; change as necessary
BATCH_SIZE = 2040

# Create the test dataloader (here we use a subset of the test set)
test_loader, test_indices = create_subset_dataloader(test_file_path, TESTSET_MAXSIZE, 196350, BATCH_SIZE, shuffle=False)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        if DEBUG:
            print(f"DEBUG: PatchEmbed input shape: {x.shape} (Expected: [B, {NUM_CHANNELS}, {IMAGE_SIZE}, {IMAGE_SIZE}])")
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self._init_weights()
        if DEBUG:
            print(f"DEBUG: Initialized VisionTransformer with embed_dim: {embed_dim}, depth: {depth}, num_heads: {num_heads}")
...
# We use the same criterion (BCELoss) for testing as in training.
...
with torch.no_grad():
    for batch in test_loader:
        ...
        qp_tensor, ctu_tensor, y_flat_64, y_flat_32, y_flat_16, y_flat_valid_32, y_flat_valid_16, target = batch
        inputs = ctu_tensor.to(device).unsqueeze(1)  # [B, 1, 64, 64]
        qp_tensor = qp_tensor.to(device)              # [B]
        ...
        # print(f"TIme taken for batch size of {BATCH_SIZE} is {time_taken}")
        ...
        time_taken = end_time - start_time
        print(f"CPU Time taken for batch size of {BATCH_SIZE} is {time_taken}")

        loss = criterion(outputs, target)
        total_test_loss += loss.item()
        
        # Compute accuracy using our helper function
...
avg_test_acc = total_avg_acc / len(test_loader)
avg_acc_l1 = total_acc_l1 / len(test_loader)
avg_acc_l2 = total_acc_l2 / len(test_loader)
avg_acc_l3 = total_acc_l3 / len(test_loader)


print("Test Loss: {:.4f}, Test Accuracy: {:.2f}%".format(avg_test_loss, avg_test_acc))
print(f"Testing Loss: {avg_test_loss:.4f} "
          f"Accuracy: {avg_test_acc:.2f}% | L1: {avg_acc_l1:.2f}% | L2: {avg_acc_l2:.2f}% | L3: {avg_acc_l3:.2f}%")
