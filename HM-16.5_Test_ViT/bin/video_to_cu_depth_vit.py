...
def get_prob(yuv_name, image_size, save_file, qp_seq, n_frames_start, n_frames_end, frame_width, frame_height):
    ...
    prob = np.zeros(
        (n_frames * int(math.ceil(frame_height / float(image_size))) * int(math.ceil(frame_width / float(image_size))),
         21))
    f = open(yuv_name, 'rb')
...
# The function call now correctly passes only the single required argument.
nt_vit.initialize_vit_predictor(model_path)
