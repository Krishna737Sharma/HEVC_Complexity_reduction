...
NUM_EXT_FEATURES = 1  # only 1 external feature input to full connecting layers, i.e., QP
NUM_LABEL_BYTES = 16
...
def net(x, y_, qp, isdrop, global_step, learning_rate_init, momentum, decay_step, decay_rate):
    ...
    h_conv_flat = tf.concat(
        values=[h_conv3_S_flat, h_conv3_M_flat, h_conv3_L_flat, h_conv2_S_flat, h_conv2_M_flat, h_conv2_L_flat], axis=1)
    print(h_conv_flat)
    ...
    h_fc1_64 = full_connect(h_conv_flat, NUM_CONVLAYER_FLAT_FILTERS, NUM_DENLAYER1_FEATURES_64,
                            acti_mode=acti_mode_fc, keep_prob=1 - isdrop * 0.5, name_w='h_fc1__64__w',
                            name_b='h_fc1__64__b')
    ...
    h_fc2_64 = full_connect(h_fc1_64, NUM_DENLAYER1_FEATURES_64 + NUM_EXT_FEATURES, NUM_DENLAYER2_FEATURES_64,
                            acti_mode=acti_mode_fc, keep_prob=1 - isdrop * 0.2, name_w='h_fc2__64__w',
                            name_b='h_fc2__64__b')
    ...
    h_fc1_32 = full_connect(h_conv_flat, NUM_CONVLAYER_FLAT_FILTERS, NUM_DENLAYER1_FEATURES_32,
                            acti_mode=acti_mode_fc, keep_prob=1 - isdrop * 0.5, name_w='h_fc1__32__w',
                            name_b='h_fc1__32__b')
    ...
    h_fc2_32 = full_connect(h_fc1_32, NUM_DENLAYER1_FEATURES_32 + NUM_EXT_FEATURES, NUM_DENLAYER2_FEATURES_32,
                            acti_mode=acti_mode_fc, keep_prob=1 - isdrop * 0.2, name_w='h_fc2__32__w',
                            name_b='h_fc2__32__b')
    ...
    h_fc1_16 = full_connect(h_conv_flat, NUM_CONVLAYER_FLAT_FILTERS, NUM_DENLAYER1_FEATURES_16,
                            acti_mode=acti_mode_fc, keep_prob=1 - isdrop * 0.5, name_w='h_fc1__16__w',
                            name_b='h_fc1__16__b')
    ...
    h_fc2_16 = full_connect(h_fc1_16, NUM_DENLAYER1_FEATURES_16 + NUM_EXT_FEATURES, NUM_DENLAYER2_FEATURES_16,
                            acti_mode=acti_mode_fc, keep_prob=1 - isdrop * 0.2, name_w='h_fc2__16__w',
                            name_b='h_fc2__16__b')
    ...
    loss_64_mean_pos = tf.reduce_sum(- tf.multiply(y_flat_64, tf.log(y_conv_flat_64 + 1e-12))) / (
                tf.to_float(tf.count_nonzero(y_flat_64)) + 1e-12)
    loss_64_mean_neg = tf.reduce_sum(- tf.multiply((1 - y_flat_64), tf.log((1 - y_conv_flat_64) + 1e-12))) / (
                tf.to_float(tf.count_nonzero(1 - y_flat_64)) + 1e-12)
    loss_64 = (loss_64_mean_pos + loss_64_mean_neg) / 2

    loss_32_mean_pos = tf.reduce_sum(
        tf.multiply(- tf.multiply(y_flat_32, tf.log(y_conv_flat_32 + 1e-12)), y_flat_valid_32)) / (
                                   tf.to_float(tf.count_nonzero(tf.multiply(y_flat_32, y_flat_valid_32))) + 1e-12)
    loss_32_mean_neg = tf.reduce_sum(
        tf.multiply(- tf.multiply((1 - y_flat_32), tf.log((1 - y_conv_flat_32) + 1e-12)), y_flat_valid_32)) / (
                                   tf.to_float(tf.count_nonzero(tf.multiply((1 - y_flat_32), y_flat_valid_32))) + 1e-12)
    loss_32 = (loss_32_mean_pos + loss_32_mean_neg) / 2

    loss_16_mean_pos = tf.reduce_sum(
        tf.multiply(- tf.multiply(y_flat_16, tf.log(y_conv_flat_16 + 1e-12)), y_flat_valid_16)) / (
                                   tf.to_float(tf.count_nonzero(tf.multiply(y_flat_16, y_flat_valid_16))) + 1e-12)
    loss_16_mean_neg = tf.reduce_sum(
        tf.multiply(- tf.multiply((1 - y_flat_16), tf.log((1 - y_conv_flat_16) + 1e-12)), y_flat_valid_16)) / (
                                   tf.to_float(tf.count_nonzero(tf.multiply((1 - y_flat_16), y_flat_valid_16))) + 1e-12)
    ...
    train_step = tf.train.MomentumOptimizer(learning_rate_current, momentum).minimize(total_loss)

    correct_prediction_valid_32 = tf.multiply(y_flat_valid_32,
                                              tf.cast(tf.equal(tf.round(y_conv_flat_32), tf.round(y_flat_32)),
                                                      tf.float32))
    correct_prediction_valid_16 = tf.multiply(y_flat_valid_16,
                                              tf.cast(tf.equal(tf.round(y_conv_flat_16), tf.round(y_flat_16)),
                                                      tf.float32))
    correct_prediction_64 = tf.equal(tf.round(y_conv_flat_64), tf.round(y_flat_64))
    accuracy_16 = tf.reduce_sum(tf.multiply(y_flat_valid_16, tf.cast(correct_prediction_valid_16, "float"))) / (
                tf.reduce_sum(y_flat_valid_16) + 1e-12)
    accuracy_32 = tf.reduce_sum(tf.multiply(y_flat_valid_32, tf.cast(correct_prediction_valid_32, "float"))) / (
                tf.reduce_sum(y_flat_valid_32) + 1e-12)
    ...
    return y_flat_64, y_flat_32, y_flat_16, y_conv_flat_64, y_conv_flat_32, y_conv_flat_16, total_loss, loss_list, learning_rate_current, train_step, accuracy_list, opt_vars_all